...
window.addEventListener('load', async () => {
  ...
  if (window.ethereum === undefined) {
    ...
    for (const token of chain.tokens) {
      ...
      if (chain.chart !== undefined) {
        token.chartLink = `${chain.chart}${token.address}`;
      }
